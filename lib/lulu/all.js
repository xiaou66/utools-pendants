...
(function (doc, win) {
    ...
    var U = function (a, b) {
        ...
        var d = 0;
        var e;
        for (d; d < a.length; d += 1) a.charCodeAt(d) >= 55296 && a.charCodeAt(d) <= 56319 ? (e = (65536 + 1024 *
            (Number(a.charCodeAt(d)) - 55296) + Number(a.charCodeAt(d + 1)) - 56320).toString(16), d +=
            1) : e = a.charCodeAt(d).toString(16),
            c += b + e;
    ...
    var tips = function (arrEles) {
        ...
        arrEles.forEach(function (ele) {
            ...
            div.setAttribute('style', 'top:' + (bounding.top + scrollTop) + 'px;left:' + (bounding.left +
                scrollLeft) + 'px;');
    ...
    doc.addEventListener('keydown', function (event) {
        ...
        if (tagName == 'textarea' || (tagName == 'input' &&
            /checkbox|radio|select|file|button|image|hidden/i.test(eleTarget.type) == false)) {
        ...
        arrAcceeekey.forEach(function (accesskey, index) {
            if ((event.key && event.key == accesskey) || (event.keyIdentifier && parseInt(event.keyIdentifier
                .toLowerCase().replace('u+', ''), 16) == parseInt(U(accesskey), 16))) {
                indexMatch = index;
                return false;
            }
        });
        ...
        } else if (arrElesOwnAccesskey[indexMatch] && !isTargetInputable && (browser == 'ie' || browser ==
            'moz') && event.altKey && !event.shiftKey && !event.ctrlKey) {
    ...
    doc.addEventListener('keydown', function (event) {
        ...
        if (keyName == 'enter' && (/^radio|checkbox$/i.test(trigger.type) || trigger.getAttribute(
            'tabindex') == '0')) {
            trigger.click();
            // 阻止默认的回车行为，主要是表单提交
            event.preventDefault();
        ...
        if (keyName == 'esc' && eleEscAll.length) {
            [].slice.call(eleEscAll).forEach(function (eleEsc) {
                var idEsc = eleEsc.id;
                eleFirstMatchAttrTarget = idEsc && doc.querySelector('[data-target="' + idEsc +
                    '"],[data-target2="' + idEsc + '"],ui-drop[target="' + idEsc + '"]');
                if (eleFirstMatchAttrTarget && eleEsc.style.display !== 'none' && eleEsc.clientHeight >
                    0) {
        ...
        if (keyName == 'tab') {
            ...
            if (!target.getAttribute('tabindex')) {
                eleFirstFocusable = target.querySelector(
        ...
        // ESC退出
        if (keyName == 'esc') {
            eleFirstMatchAttrTarget = doc.querySelector('a[data-target="' + attrTarget +
                '"],input[data-target="' + attrTarget + '"]');
            if (attrFocus) {
                trigger.blur();
        ...
        // 如果都符合，同时有目标子元素
        var arrEleFocusable = [].slice.call(target.querySelectorAll(
            'a[href], button:not(:disabled), input:not(:disabled)'));
        var index = -1;
    ...
    doc.addEventListener('click', function (event) {
        ...
        // 当然，本身outline不是none
        if (target && target == eleActive && (/^radio|checkbox$/i.test(eleActive.type) || tabindex >= 0) &&
            win.isKeyEvent == false && /none/.test(getComputedStyle(target).outline) == false) {
            eleActive.blur();
        }
    });
    // 全局对a标签按钮进行无障碍角色设置
...
HTMLElement.prototype.follow = function (eleTarget, options) {
    ...
    if (!eleTarget) {
        ...
        eleTarget = document.getElementById(strTarget) || document.querySelector('.' + strTarget) || document.querySelector(
            strTarget);
        if (!eleTarget) {
            return;
        }
    ...
    if ((objBoundTrigger.left < 0 && objBoundTrigger.right < 0) || (objBoundTrigger.top < 0 && objBoundTrigger.bottom <
        0) || (objBoundTrigger.left > numWinWidth && objBoundTrigger.right > numWinWidth) || (objBoundTrigger.top >
        numWinHeight && objBoundTrigger.bottom > numWinHeight)) {
    ...
    // 自动调整距离边缘的安全距离
    let arrSafeArea = this.dataset.safeArea || getComputedStyle(eleTarget).getPropertyValue('--safe-area') ||
        objParams.safeArea;
    // 字符串转数组
    ...
    const objIsOverflow = {
        ...
        'right-left': objBoundTrigger.right + objBoundTarget.width + objOffsets.x + arrSafeArea[1] >
            numWinWidth,
        'bottom-top': objBoundTrigger.bottom + objBoundTarget.height + objOffsets.y + arrSafeArea[2] >
            numWinHeight,
        // 新增4个方位
...
class Tab extends HTMLElement {
    ...
    switch () {
        ...
        if (this.history == true && strName && /tab\d{10,16}/.test(strName) == false) {
            ...
            // 改变当前URL
            window.history.replaceState(null, document.title, location.href.split('?')[0] + '?' + objURLParams.toString() +
                strHash);
        }
    }
    // 自动切换
    ...
    connectedCallback() {
        ...
        objURLParams.forEach((value, key) => {
            if (eleTrigger && eleTarget && this.name == key && eleTarget.id == value && !eleTrigger.hasAttribute(
                'open')) {
                eleTrigger.click();
            }
        });
...
class Select extends HTMLSelectElement {
    ...
    setWidth() {
        if (this.element.combobox) {
            ...
            if (width.lastIndexOf('%') !== -1 && this.originPosition != 'absolute' && this.originPosition !=
                'fixed') {
                // 如果是百分比宽度
                // 同时原下拉框不是绝对定位
                ...
                // 避免水平滚动的出现
                this.style.transform =
                    `scaleX(${ this.parentElement.clientWidth * parseFloat(width) * 0.01 / this.offsetWidth })`;
            }
        }
    }
    create() {
        ...
        this.insertAdjacentHTML('afterend',
            `<div role="combobox" style="width: ${this.getWidth()}">
           ${!this.multiple ? `<a
                class="${BUTTON_CLASS}"
                data-target="${strId}"
                aria-owns="${strId}"
    ...
    refresh(data) {
        ...
        if (isMultiple) {
            ...
            // 按钮赋值
            eleButton.innerHTML =
                `<span class="${Select.addClass('text')}">${strHtmlSelected}</span><i class="${Select.addClass('icon')}" aria-hidden="true"></i>`;
        }
        // 列表内容的刷新
        let index = -1;
            ...
            document.addEventListener('click', (event) => {
                ...
                if (eleButton.contains(target)) {
                    ...
                    if (eleCombobox.classList.contains('active')) {
                        // 边界判断
                        var isOverflow = eleDatalist.getBoundingClientRect().bottom + window.pageYOffset >
                        ...
                        // 严格验证
                        if (arrDataScrollTop && eleDatalistSelected && arrDataScrollTop[1] ===
                            eleDatalistSelected.getAttribute('data-index') && arrDataScrollTop[2] ===
                            eleDatalistSelected.innerText) {
...
class Drop extends HTMLElement {
    ...
    events(isIgnoreTarget) {
            ...
            case 'mouseenter':
            {
                ...
                if (!eleTrigger.isBindDropEvents) {
                    ...
                    eleTrigger.addEventListener('mouseout', (event) => {
                        ...
                        if (this.element.trigger == event.target || this.element.trigger.contains(event
                            .target)) {
                            // 清除显示的定时器
        ...
        if (objParams.eventType != 'null' && !eleTrigger.isBindDocMouseUp) {
            document.addEventListener('mouseup', event => {
                ...
                let eleTrigger = this.element.trigger;
                let eleTarget = this.element.target;
                if (eleTrigger.contains(eleClicked) == false && (!eleTarget || eleTarget.contains(
                    eleClicked) == false)) {
                    this.hide();
                }
            });
    ...
    list(eleTrigger, data, options) {
            ...
            eleTarget.innerHTML = (() => {
                ...
                const funStep = (arrData, arrIndex) => {
                    ...
                    arrData.forEach((objData, numIndex) => {
                        ...
                        if (isSomeItemSelected == false && strMatchIndex == arrCurrentIndex
                            .join('-')) {
                            objData[SELECTED] = true;
                        }
                        // 类名
                        ...
                        if (objData.heading == true) {
                            ...
                            strHtmlStep += '<div class="' + strAttrClass + '"' +
                                strAttrLabel + ' role="heading">' + objData.value +
                                '</div>';
                            return;
                        }
                        // 禁用态和非禁用使用标签区分
                        ...
                        if (objData[DISABLED] != true) {
                            strHtmlStep += '<a href="' + strAttrHref + '"' + strAttrTarget +
                                strAttrLabel + strAttrAccess + ' class="' + strAttrClass +
                                '" data-index="' + arrCurrentIndex.join('-') +
                                '" role="option" aria-selected="' + (objData[SELECTED] ||
                                    'false') + '" ' + strAttrSublist + '>' + objData.value +
                                '</a>';
                            if (objData.data) {
                                strHtmlStep += '<div class="' + CL.add('xx') +
                                    '"><div class="' + CL.add('x') + '" role="listbox">' +
                                    funStep(objData.data, arrCurrentIndex) + '</div></div>';
                            }
                        } else {
                            strHtmlStep += '<span class="' + strAttrClass + '"' +
                                strAttrLabel + strAttrAccess + '>' + objData.value +
                                '</span>';
                        }
                    });
                    return strHtmlStep;
        ...
        eleTarget.addEventListener('mouseover', event => {
            ...
            if (eleHovered.classList.contains(SELECTED) == false && typeof eleHovered.getAttribute(
                'data-sublist') === 'string') {
                eleHovered.click();
            }
        });
    ...
    panel(eleTrigger, options) {
        ...
        objParams.buttons.forEach((objBtn, numIndex) => {
            ...
            // 按钮类名
            eleBtn.className = String(CL).replace('dropanel', 'button') + ' ' + CL.add('button') + ' ' + (
                objBtn.className || '');
            // 按钮的类型
            if (strType) {
    ...
    Object.defineProperty(document, 'loading', {
        ...
        set(newValue) {
            if (newValue) {
                ...
                // loading 文字显示
                let numIndex = 0;
                let arrTips = ['正在加载中<ui-dot>...</ui-dot>', '仍在加载中<ui-dot>...</ui-dot>',
                    '请再稍等片刻<ui-dot>...</ui-dot>'];
                if (typeof newValue == 'string') {
                    arrTips = [newValue];
            ...
            Object.defineProperties(dialog, {
                ...
                alert: {
                    value: function (content, options) {
                        ...
                        if (objParams.buttons.length && !objParams.buttons[0].type) {
                            ...
                            if (/^remind|success|warning|danger$/.test(objParams.type) ===
                                false) {
                                objParams.buttons[0].type = defaults.type;
                            }
                        }
                        let nodes = new DOMParser().parseFromString(strContent, 'text/html').body
                            .childNodes;
                        if (nodes.length == 1) {
                            // 如果是纯文本
                            if (nodes[0].nodeType === 3) {
                                strContent = '<p class="' + CL.add('wrap') + '">' + strContent +
                                    '</p>';
                            }
                        } else {
                            strContent = '<div class="' + CL.add('wrap') + '">' + strContent +
                                '</div>';
                        }
                        // 主体内容更新
                        strContent = '<div class="' + CL.add(objParams.type) + ' ' + CL.add(
                            'alert') + '">' + strContent + '</div>';
                        this.setParams({
                            width: 'auto',
                            title: objParams.title,
                            buttons: objParams.buttons,
                ...
                content: {
                    ...
                    set(content) {
                        ...
                        if (typeof content == 'function') {
                            content = content();
                        } else if (typeof content == 'string' && /^#?\w+(?:[-_]\w+)*$/i.test(
                            content)) {
                            // 如果是字符串
                            // 如果是选择器，仅支持ID选择器
                        ...
                        if (this.closeMode == 'remove') {
                            ...
                            let isParentHidden = eleContentParent && eleContentParent.matches(
                                'div[hidden]');
                            // 弹框中显示
                            ...
                            // 如果content是隐藏的则显示
                            if (content.nodeType === 1 && getComputedStyle(content).display ==
                                'none') {
                ...
                scrollbar: {
                    value: function () {
                        ...
                        // 是否有显示的弹框
                        const isDisplayed = [].slice.call(eleAllDialog).some(function (
                            eleDialog) {
                            return window.getComputedStyle(eleDialog).display !==
                                'none';
                        });
                        ...
                        if (isDisplayed) {
                            ...
                            document.body.style.borderRight = widthScrollbar +
                ...
                tabindex: {
                    value: function () {
                        ...
                        } else if (eleLastActiveElement && eleLastActiveElement.tagName.toLowerCase() !=
                            'body') {
                            // 键盘焦点元素还原
...
const Datalist = (() => {
    ...
    class Datalist extends HTMLInputElement {
            ...
            if (this.params.data == 'auto') {
                ...
                if (strAttrList) {
                    ...
                    this.params.data = function () {
                        return [].slice.call(eleDatalist.querySelectorAll('option')).map(function (
                            eleOption) {
                            let objAttr = {};
        ...
        events() {
            ...
            if (this.form && this.params.data == 'auto' && this.name && (this.autocomplete == '' || this.autocomplete ==
                'on')) {
            ...
            document.addEventListener(objEventType.end, (event) => {
                ...
                if (eleClicked != this && this.value.trim() == '') {
                    ...
                    } else if (typeof this.params.placeholder == 'string' && this.params.placeholder !==
                        'auto') {
    ...
    class Component extends HTMLInputElement {
        ...
        events() {
            ...
            eleContainer.addEventListener('click', (event) => {
                ...
                switch (eleContainer.dataset.type) {
                    ...
                    case 'date-range':
                    {
                        ...
                        if (/prev|next/.test(eleClicked.className)) {
                            ...
                            if (dataRange[0].join() == dataRange[1].join()) {
                                ...
                                // 如果前后时间不一样，说明现在有范围
                                // 则取消范围，变成单选
                                dataRange = [[numYear, numMonth, numDay], [numYear, numMonth,
                                    numDay]];
                            }
                    ...
                    case 'time':
                    {
                        if (eleClicked.tagName == 'BUTTON' && eleClicked.classList.contains(
                            SELECTED) == false) {
            ...
            // time类型的上下左右快捷键处理
            document.addEventListener('keydown', event => {
                if (eleContainer.dataset.type == 'time' && this.display == true && eleContainer.contains(
                    ...
                    if (/^arrow/i.test(event.key)) {
                        ...
                        // 当前列所有可点击元素
                        let eleButtonClickable = [...event.target.parentElement.querySelectorAll(
                            'button:enabled:not([data-visibility="false"])')];
                        let numIndexButtonClickable = eleButtonClickable.findIndex(item => item ==
                            event.target);
                        // 上下左右快捷键的处理
        ...
        setValue() {
            ...
            switch (this.params.type) {
                ...
                {
                    this.value =
                        `${arrSelected[0].slice(0, 2).join('-')} 至 ${arrSelected[1].slice(0, 2).join('-')}`;
                    break;
                }
                case 'time':
                case 'minute':
        ...
        getCalendarData(arrDate) {
            ...
            const getStrHtmlDay = () => {
                ...
                arrChinese.forEach((strChineseDay, indexDay) => {
                    strHtmlDay =
                        `${strHtmlDay}<span class="${CL.day('item')} col${indexDay}">${strChineseDay}</span>`;
                });
                return strHtmlDay;
            };
            // 3 星期几七大罪
                    ...
                    for (let td = 0; td < 7; td++) {
                        ...
                        if (strType == 'date') {
                            ...
                            if (tr == 0 && td < numDayFirst) {
                                ...
                                // HTML拼接
                                strHtmlDate =
                                    `${strHtmlDate}<span class="${strClass}" ${strHtmlFullData}>${numDayNow}</span>`;
                            } else {
                                // 当前日子
                                ...
                                if (numDayNow <= arrMonthDay[objNewDate.getMonth()]) {
                                    ...
                                    if (objDateNow >= numMin && objDateNow <= numMax) {
                                        strHtmlDate =
                                            `${strHtmlDate}<a href="javascript:;" ${strHtmlData} class="${strClass}" ${strHtmlFullData}>${numDayNow}</a>`;
                                    } else {
                                        strHtmlDate =
                                            `${strHtmlDate}<span class="${strClass}" ${strHtmlFullData}>${numDayNow}</span>`;
                                    }
                                    ...
                                    // 更新strHtmlFullData
                                    strHtmlFullData = strHtmlYearMonthData + new Date(numYearNow,
                                        numMonthNow, numDayNow).toArray().join('-');
                                    // 日期字符拼接
                                    strHtmlDate =
                                        `${strHtmlDate}<span class="${strClass}" ${strHtmlFullData}>${numDayNow}</span>`;
                                }
            ...
            const getStrHtmlDate = () => {
                ...
                for (let i = 1; i <= 12; i += 1) {
                    ...
                    if (numYear + strMonth >= numMin && numYear + strMonth <= numMax) {
                        strHtmlDate =
                            `${strHtmlDate}<a href="javascript:" class="${strClass}" data-year="${numYear}" data-value="${strMonth}">${arrChinese[i - 1]}月</a>`;
                    } else {
                        strHtmlDate =
                            `${strHtmlDate}<span class="${strClass}" data-value="${strMonth}">${arrChinese[i - 1]}月</span>`;
                    }
                }
                return strHtmlDate;
            };
        ...
        date() {
            ...
            const datePrevMonthLastDay = new Date(numPrevYearGet, numPrevMonthGet, objCalender.monthDay[
                numPrevMonthGet]);
            // 上个月份的最大日期比最小限制大
            // 或者没有最小限制
            // 或者
            if (datePrevMonthLastDay >= objCalender.min) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('prev')}" data-month="${numPrevMonth}" role="button" aria-label="上一月"></a>`;
            } else {
            ...
            if (new Date(numNextYearGet, numNextMonthGet, 1) <= objCalender.max) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('next')}" data-month="${numNextMonth}" role="button" aria-label="下一月"></a>`;
            } else {
                strHtml = `${strHtml}<span class="${CL.date('next')}" aria-label="下一月"></span>`;
            }
            // 头部月份公用结束
            strHtml =
                `${strHtml}<a href="javascript:" class="${CL.date('switch')}" data-type="month" role="button" aria-label="快速切换月份">${arrDate.slice(0, 2).join('-')}</a></div>`;
            ...
            if (new Date() >= objCalender.min && new Date() <= objCalender.max) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('item')} ${CL.date('now')}" role="button">今天</a>`;
            } else {
        ...
        ['date-range']() {
            ...
            //  2.1 上一个月箭头
            const datePrevMonthLastDay = new Date(arrDate[0], numPrevMonth - 1, objCalender.monthDay[
                numPrevMonth]);
            // 上个月的最后1天大于min最小限制
            if (datePrevMonthLastDay >= objCalender.min) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('prev')}" data-month="${numPrevMonth}" aria-label="上一个月，当前${arrDate[1]}月"></a>`;
            } else {
                strHtml = `${strHtml}<span class="${CL.date('prev')}"></span>`;
            }
            // 今月月份显示
            strHtml =
                `${strHtml}<span class="${CL.date('switch')}">${new Date(arrDate[0], numPrevMonth, arrDate[2]).toArray().slice(0, 2).join('-')}</span></div><div class="${CL.date('half')}">`;
            // 2.2 下下个月
            ...
            if (objDateAfterMonth <= objCalender.max) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('next')}" data-month="${numNextMonth}" aria-label="下一个月，当前${numNextMonth}月"></a>`;
            } else {
                strHtml = `${strHtml}<span class="${CL.date('next')}"></span>`;
            }
            // 下月月份显示
            strHtml =
                `${strHtml}<span class="${CL.date('switch')}">${objDateNextMonth.toArray().slice(0, 2).join('-')}</span></div>`;
            // 头部闭合
            strHtml += '</div>';
            // 3. 两个主体列表
            // 这里要嵌套一层range特有的body
        ...
        month() {
            ...
            if (numYear - 1 >= Math.floor(numMin / 100) && numYear - 1 <= Math.floor(numMax / 100)) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('prev')}" data-year="${numYear - 1}" role="button" aria-label="上一年"></a>`;
            } else {
            ...
            if (numYear + 1 >= Math.floor(numMin / 100) && numYear + 1 <= Math.floor(numMax / 100)) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('next')}" data-year="${numYear + 1}" role="button" aria-label="下一年"></a>`;
            } else {
                strHtml = `${strHtml}<span class="${CL.date('next')}"></span>`;
            }
            // 头部结束
            strHtml =
                `${strHtml}<a href="javascript:" class="${CL.date('switch')}" data-type="year" role="button" aria-label="快速切换年份">${numYear}</a>\
            </div>`;
            // 3. 月份切换主体列表
            ...
            if (objThisYearMonth >= numMin && objThisYearMonth <= numMax) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('item')} ${CL.date('now')}">今月</a>`;
            } else {
        ...
        ['month-range']() {
            ...
            // 2. 头部
            strHtml =
                `${strHtml}<div class="${CL.date('head')}">\
            <div class="${CL.date('half')}">`;
            //  2.1 上一年箭头
            if (numPrevYear >= numMinYear && numPrevYear <= numMaxYear) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('prev')}" data-year="${numPrevYear}" role="button" aria-label="上一年"></a>`;
            } else {
                strHtml = `${strHtml}<span class="${CL.date('prev')}" aria-label="上一年"></span>`;
            }
            // 今年年份显示
            strHtml =
                `${strHtml}<span class="${CL.date('switch')}">${arrDate[0]}</span>\
            </div>\
            <div class="${CL.date('half')}">`;
            // 2.2 下一年
            if (numNextYear >= numMinYear && numNextYear < numMaxYear) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('next')}" data-year="${numNextYear}" role="button" aria-label="下一年"></a>`;
            } else {
            ...
            // 主体标签闭合
            strHtml =
                `${strHtml}<div class="${CL.range('body')} ${CL.range('month', 'body')}">\
        ...
        year() {
            ...
            if (numYear - 12 >= numMin && numYear - 12 <= numMax) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('prev')}" data-year="${numYear - 12}" role="button" aria-label="上一个12年"></a>`;
            } else {
                strHtml = `${strHtml}<span class="${CL.date('prev')}"></span>`;
            }
            //    2.2 下一个12年
            if (numYear + 12 >= numMin && numYear + 12 <= numMax) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('next')}" data-year="${numYear + 12}" role="button" aria-label="下一个12年"></a>`;
            } else {
                strHtml = `${strHtml}<span class="${CL.date('next')}"></span>`;
            }
            // year选择是顶级类别，一定是不可点击的
            strHtml =
                `${strHtml}<span class="${CL.date('switch')}">${[numYear - 6, numYear + 5].join('-')}</span></div>`;
            const getStrHtmlDate = () => {
                let strHtmlDate = '';
                let strClass = '';
                for (let indexYear = numYear - 6; indexYear < numYear + 6; indexYear += 1) {
                    ...
                    if (indexYear >= numMin && indexYear <= numMax) {
                        strHtmlDate =
                            `${strHtmlDate}<a href="javascript:" class="${strClass}">${indexYear}</a>`;
                    } else {
            ...
            if (numThisYear >= numMin && numThisYear <= numMax) {
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('item')} ${CL.date('now')}" role="button">今年</a>`;
            } else {
            ...
            const getStrHtmlTime = () => {
                ...
                for (let indexHour = 0; indexHour < 24; indexHour += numStep) {
                    ...
                    if (indexHour >= numMin && indexHour <= numMax) {
                        strHtmlTime =
                            `${strHtmlTime}<a href="javascript:" class="${strClass}">${strHour}:00</a>`;
                    } else {
        ...
        time() {
            ...
            let arrSeconds = Array(60).fill().map((empty, index) => {
                ...
                // 是否选中
                let selected = false;
                if (numHourSelected == numMinHour && numMinuteSelected == numMinMinute && index <
                    numMinSecond) {
                    disabled = true;
                } else if (numHourSelected == numMaxHour && numMinuteSelected == numMaxMinute &&
                    index > numMaxSecond) {
                    disabled = true;
                }
                // step的处理
            ...
            strHtml = strHtml +
                `<div class="${CL.time('col')}" data-type="minute">
            ...
            if (arrTime.length == 3) {
                strHtml = strHtml +
                    `<div class="${CL.time('col')}" data-type="second">
        ...
        minute() {
            ...
            if (hour <= Math.floor(numMin / 100)) {
                ...
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('prev')}" data-hour="${hour - 1}" role="button" aria-label="上一个小时"></a>`;
            }
            // 2.2 可不可以往后翻
            if (hour >= Math.floor(numMax / 100)) {
                ...
                strHtml =
                    `${strHtml}<a href="javascript:" class="${CL.date('next')}" data-hour="${hour + 1}" role="button" aria-label="下一个小时"></a>`;
            }
            // 头部结束的公共html部分
            strHtml =
                `${strHtml}<a href="javascript:" class="${CL.date('switch')}" data-type="hour">${arrTime[0]}:00</a></div>`;
            const getStrHtmlTime = () => {
                let strHtmlTime = '';
                let strMinute = '';
                let strClass = '';
                ...
                for (let indexMinute = 0; indexMinute < 60; indexMinute += numStep) {
                    ...
                    strClass = CL.date('item');
                    // 是否在时间范围内
                    if ((arrTime[0] + strMinute) * 1 >= numMin && (arrTime[0] + strMinute) * 1 <=
                        numMax) {
                        // 选中态
                        ...
                        strHtmlTime =
                            `${strHtmlTime}<a href="javascript:" class="${strClass}">${[arrTime[0], strMinute].join(':')}</a>`;
                    } else {
                        strHtmlTime =
                            `${strHtmlTime}<span class="${strClass}">${[arrTime[0], strMinute].join(':')}</span>`;
                    }
                }
                return strHtmlTime;
            };
        ...
        show() {
            ...
            if (this.params.type == 'time') {
                [...eleContainer.querySelectorAll('.' + SELECTED)].forEach((item, index) => {
                    ...
                    } else if (item.offsetTop - 5 > item.parentElement.scrollTop + item.parentElement
                        .clientHeight || item.offsetTop - 5 < item.parentElement.scrollTop) {
        ...
        connectedCallback() {
            ...
            if (['date', 'year', 'month', 'time', 'hour', 'minute', 'date-range', 'month-range'].includes(
                strType) == false) {
                strType = 'date';
            ...
            switch (strType) {
                ...
                case 'month-range':
                {
                    ...
                    if (strType == 'date-range') {
                        ...
                        this.value =
                            `${arrBegin.slice(0, 2).join('-')} 至 ${arrEnd.slice(0, 2).join('-')}`;
                    }
                    // 存储
...
const Validate = (() => {
        ...
        return {
            ...
            getReportText(element) {
                ...
                const strName = defaultPrompt.name[strType] || (function () {
                    ...
                    let strTextLabel = '';
                    // 从label动态获取提示数据
                    document.querySelectorAll('label[for=' + strId + ']').forEach(function (
                        eleLabel) {
                        ...
                        // 去除数字和冒号
                        const strLabelCloneText = eleLabelClone.innerHTML.trim().replace(
                            /\d/g, '').replace('：', '');
                        // 内容最长的那个元素作为提示文字
                ...
                if (objValidateState.valueMissing) {
                    ...
                    if (!strFinalText) {
                        // 然后确定是否是多值
                        const isMultiple = element.hasAttribute('multiple') && element.value.split(',')
                            .length > 1;
                        strFinalText = defaultPrompt.patternMismatch[isMultiple ? 'multiple' :
                            'pattern'];
                        // 然后
                        if (strType && strName) {
                            // 1. 试试关键字提示
                    ...
                    if (!strFinalText) {
                        ...
                        if (strType == 'number' || strType == 'range') {
                            ...
                            strFinalText = '请' + (element.hasAttribute('readonly') ? '选择' : '输入') +
                                '有效的值。' + (strName || '') + '间隔是' + numStep;
                        }
                    }
                    ...
                    if (objValidateState.tooLong) {
                        ...
                        if (!strFinalText) {
                            ...
                            strFinalText = (strName || '') + '内容长度不能大于' + strMaxLength.replace(/\D/g,
                                '') + strTextCharLength;
                        }
                    } else {
                        // 先看看有没有自定义的提示
            ...
            */
                ...
                if (element.hasAttribute('required')) {
                    ...
                    if (strType == 'radio') {
                        ...
                        if (element.name && eleForm) {
                            eleRadios = eleForm.querySelectorAll('input[type="radio"][name="' + element
                                .name + '"]');
                        }
                        // 如果单选框们没有一个选中，则认为无视了required属性
                        const isAtLeastOneRadioChecked = [].slice.call(eleRadios).some(function (
                            eleRadio) {
            ...
            getRangeState(element) {
                ...
                if (strType.slice(-6) != '-range') {
                    ...
                    if ((strType == 'hour' || strType == 'minute' || strType == 'time') && strAttrMin &&
                        strAttrStep) {
                        const minuteValue = strValue.split(':')[1];
                        const minuteMin = strAttrMin.split(':')[1];
                        if (strType == 'hour' && (minuteValue != minuteMin || (strValue.split(':')[0] -
                            strAttrMin.split(':')[0]) % strAttrStep != 0)) {
            ...
            isOut(element) {
                ...
                return objValidateState.badInput || objValidateState.rangeOverflow || objValidateState.rangeUnderflow ||
                    objValidateState.stepMismatch;
            },
            /**
             * 内容是否超出长度限制的判断
            ...
            getValidity(element) {
                ...
                // 且不是单复选框，则直接返回
                // 避免同一元素短时间多次验证
                if (element.lastValidateState && element.lastValue === element.value &&
                    /radio|checkbox/.test(element.type) == false) {
            ...
            styleError(element, valid) {
                ...
                const eleForm = element.form || element.closest('form') || (element.customValidate &&
                    element.customValidate.owner);
                if (element.type == 'radio' && element.name && eleForm) {
                    // 单选框组是整体去除高亮
                    eleForm.querySelectorAll('input[type=radio][name=' + element.name + ']').forEach(
                        function (eleRadio) {
                ...
                const funShow = function () {
                    ...
                    eleTipTarget.errorTip(content, {
                        onShow(eleTrigger, eleTips) {
                            ...
                            if (numOffsetX < 0) {
                                eleTips.style.marginLeft = 0.5 * (eleTips.clientWidth -
                                    eleTipTarget.clientWidth) + 'px';
                            } else {
                    ...
                    if (content.indexOf('内容长度') != -1 && content.indexOf('大') != -1) {
                        ...
                        // }
                        // 获得最大长度的索引位置
                        const strAttrMaxLength = eleControl.maxlength || eleControl.getAttribute(
                            'maxlength').replace(/\D/g, '');
                        if (numLength && strAttrMaxLength) {
                            document.validate.selectRange(element, document.validate.getLength(
                                element, strAttrMaxLength), numLength);
                        }
            ...
            getTarget(element) {
                ...
                if (strType == 'radio') {
                    ...
                    if (objStyle.opacity != '1') {
                        eleTarget = element.parentElement.querySelector('label.ui-checkbox[for="' +
                            strId + '"]');
                    }
                    // 下拉框
                    ...
                    // 隐藏元素的目标提示元素的转移
                } else if (strType == 'hidden' || objStyle.display == 'none' || objStyle.visibility ==
                    'hidden') {
                    const eleTargetRel = document.getElementById(eleTarget.getAttribute('data-target')) ||
                        element.dataTarget;
                    if (eleTargetRel) {
                        eleTarget = document.validate.getTarget(eleTargetRel);
                    }
                } else if (strType == 'textarea' || strTag == 'textarea') {
                    if (element.classList.contains('ui-textarea') == false && element.parentElement.querySelector(
                        '.ui-textarea')) {
                        eleTarget = element.parentElement.querySelector('.ui-textarea');
                    }
                } else if (strTag == 'input') {
                    if (element.classList.contains('ui-input') == false && element.parentElement.querySelector(
                        '.ui-input')) {
    ...
    class Component {
        ...
        constructor(element, callback, options) {
            ...
            const objParams = Object.assign({}, defaults, options || {});
            // 还原禁用的提交和关闭按钮
            eleForm.querySelectorAll('[type="submit"]:disabled, [type="image"]:disabled').forEach(function (
                eleSubmit) {
        ...
        setCustomValidity(validate) {
            ...
            // 遍历元素并匹配
            let eleFormCollection = this.element.form.elements || this.element.form.querySelectorAll(
        ...
        count() {
            ...
            eleForm.querySelectorAll('input, textarea').forEach(function (element) {
                ...
                // 有没有label元素
                let eleLabel = element.parentElement.querySelector('.' + CL.add('count')) ||
                    eleForm.querySelector('.' + CL.add('count') + '[for="' + element.id + '"]');
                ...
                if (!eleLabel) {
                    ...
                    eleLabel.innerHTML = '<span>0</span><slash>/</slash><span>' + (strAttrMinLength ?
                        strAttrMinLength + '-' : '') + strAttrMaxLength + '</span>';
                    // 插入到后面
                ...
                const funCount = function () {
                    ...
                    // 超出范围或范围不足
                    if (length != 0 && (length > strAttrMaxLength || (strAttrMinLength &&
                        length < strAttrMinLength))) {
                ...
                if (/^checkbox|radio|range$/i.test(strAttrType) == false) {
                    ['paste', 'drop'].forEach(eventType => {
                        element.addEventListener(eventType, function (event) {
                            ...
                            // 获取选中的文本内容
                            let textSelected = this.value.slice(element.selectionStart,
                            ...
                            if (this.value.trim() == '' || textSelected === this.value) {
                                ...
                                if (type == 'email') {
                                    ...
                                    // 手机号处理
                                    strPassText = document.validate.getTel(
                                        strPassText);
                                }
                                // 插入
        ...
        immediate() {
            ...
            eleForm.querySelectorAll('input, select, textarea').forEach(function (element) {
                ...
                // 给每个控件绑定即时验证
                if (strType == 'button' || strType == 'submit' || strType == 'reset' || strType ==
                    'file' || strType == 'image') {
                    return;
                }
                // 不同类别不同事件
                ...
                } else if (/select/.test(strType) || /range|date|time|hour|minute|hidden/.test(
                    strAttrType)) {
    ...
    updatePage(current = this.current) {
        ...
        if (this.simple) {
            ...
            switch (current) {
                ...
                case this.count - 3:
                case this.count - 4:
                    place = [1, 'pre', this.count - 6, this.count - 5, this.count - 4, this.count - 3, this.count -
                    2, this.count - 1, this.count];
                    break;
                default:
...
const Table = (function () {
    ...
    class Table extends HTMLTableElement {
        ...
        setParams(options) {
            ...
            if (objPageOption) {
                objPageOption.keyMap = Object.assign({}, this.params.page.keyMap || Table.defaultKeyMap,
                    objPageOption.keyMap || {});
            }
        ...
        events() {
            ...
            this.addEventListener('click', function (event) {
                ...
                this.querySelectorAll('tr').forEach(function (tr) {
                    ...
                    // 所有非禁用复选框
                    let eleTdCheckbox = tr.querySelector('td:first-child ' + CL.checkbox +
                        ':enabled');
                    if (eleTdCheckbox) {
                ...
                if (eleTh) {
                    ...
                    let numLengthChecked = [].slice.call(eleAllTdCheckbox).filter(function (
                        eleTdCheckbox) {
                ...
                // 根据复选框状态决定表格行样式
                eleAllTdCheckbox.forEach(function (eleTdCheckbox) {
                    eleTdCheckbox.closest('tr').classList[eleTdCheckbox[CHECKED] ? 'add' :
                        'remove'](SELECTED);
                });
                // 回调
            ...
            this.addEventListener('click', function (event) {
                ...
                if (eleTarget && /^(?:a|input|textarea|tbody|i|select|label|th)$/i.test(eleTarget.tagName) ==
                    false) {
                    eleCheckbox = eleTarget.closest('tr') && eleTarget.closest('tr').querySelector(
                        'td:first-child ' + CL.checkbox + ':enabled');
                    if (eleCheckbox) {
                        eleCheckbox.click();
                    }
                }
        ...
        ajax(options) {
            ...
            let funAjax = () => {
                ...
                xhr.onload = () => {
                    ...
                    if (!strHtml || !strHtml.trim()) {
                        if (!eleEmpty) {
                            eleEmpty = document.querySelector('.' + CL.empty) || document.createElement(
                                'div');
                    ...
                    if (numTotal || numTotal == 0) {
                        eleContainer.querySelectorAll('output[data-type="total"]').forEach(function (
                            eleTotal) {
            ...
            if (window.getComputedStyle(eleLoading).display == 'none') {
                ...
                eleLoading.style.height = (this.clientHeight - (eleThead ? eleThead.clientHeight : 0)) +
                    'px';
